...
class StoryVariables
{
    ...
    public function GetStoryVariables()
    {
        ...
        $room_id = (int)Request::param('room_id', 0);

        if ($room_id <= 0) {
            return error('房间ID不能为空或无效', 400);
        }

        // 查询该 room_id 下的所有变量
        $list = Db::name('room_story_variables')
            ->where('room_id', $room_id)
            ->select()
            ->toArray();

        return success($list, '房间剧情变量获取成功');
    }
    ...
    public function SaveStoryVariable()
    {
        ...
        $room_id = (int)$params['room_id'] ?? 0;
        $variable_key = (string)($params['variable_key'] ?? '');
        // $operators   = (int)$params['operators'] ?? 0;
        ...
        try {
            if ($id > 0) {
                // 更新：根据 id 查找并更新记录
                $exist = Db::name('room_story_variables')->where('id', $id)->find();
                if (!$exist) {
                    return error('要更新的剧情变量不存在', 404);
                }

                // 可选：检查 room_id 是否被修改，避免越权
                if ((int)$exist['room_id'] !== $room_id) {
                    return error('不允许修改房间ID', 403);
                }

                $result = Db::name('room_story_variables')
                    ->where('id', $id)
                    ->update($data);
                ...
                $id = Db::name('room_story_variables')->insertGetId($data);
